...
public class ScaleDownImageTransition extends Transition {
    ...
    private static final String PROP_NAME_SCALE_X = "transitions:scale_down:scale_x";
    private static final String PROP_NAME_SCALE_Y = "transitions:scale_down:scale_y";
    ...
    public Animator createAnimator( final ViewGroup sceneRoot, final TransitionValues startValues, final TransitionValues endValues ) {
        ...
        if ( view instanceof ImageView ) {
            ...
            if ( startValues != null ) {
                scaleX = (float) startValues.values.get( PROP_NAME_SCALE_X );
                scaleY = (float) startValues.values.get( PROP_NAME_SCALE_Y );
            }

            float targetScaleX = (float) endValues.values.get( PROP_NAME_SCALE_X );
            float targetScaleY = (float) endValues.values.get( PROP_NAME_SCALE_Y );
    ...
    private void captureValues( final TransitionValues values, final float scaleX, final float scaleY ) {
        values.values.put( PROP_NAME_SCALE_X, scaleX );
        values.values.put( PROP_NAME_SCALE_Y, scaleY );
    }
}